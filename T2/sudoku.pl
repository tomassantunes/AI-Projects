...
ve_restricoes(e(_, I)) :- verifica_linha(I), verifica_coluna(I), verifica_quadrantes(I).

verifica_linha(e(_Nafect,[v(p(X,_Y), _D, V)|R])):-
    findall(V1,member(v(p(X,_),_,V1),R),L),
    todos_diff([V|L]).

verifica_coluna(e(_Nafect,[v(p(_X,Y), _D, V)|R])):-
    findall(V1,member(v(p(_,Y),_,V1),R),L),
    todos_diff([V|L]).

verifica_quadrantes(e(_,Afect)) :-
